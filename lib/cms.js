...
import { compareCategories } from './categoryOrder.js';
...
export const defaultSettings = {
  ...
  languages: {
    ...
    ar: { label: 'العربية', enabled: false, direction: 'rtl' }
  },
  features: {
    blog: true,
    pages: true,
...
export function localizedHref(href, lang) {
  ...
  const url = new URL(href, 'https://yallatogether.com');
...
export async function getTerms(type, options = {}) {
  ...
  if (Array.isArray(collection)) return collection.sort(compareCategories);

  const query = { type };
  if (!includeDisabled) query.enabled = { $ne: false };
  return (await collection.find(query).toArray()).sort(compareCategories);
}
...
export function groupCategories(categories, lang) {
  const enabled = categories.filter((category) => category.enabled !== false).sort(compareCategories);
...
export function renderContentItem(item, lang) {
  ...
  return {
    ...
    dateText: item.publishedAt ? formatDate(item.publishedAt, lang) : ''
  };
}
